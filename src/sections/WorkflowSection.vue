<template>
  <section id="workflow" class="bg-white py-24">
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
      ...
      <div class="mt-16 grid gap-10 lg:grid-cols-[1.3fr_1fr]">
        <div class="grid gap-6">
          <article
            v-for="(step, index) in steps"
            :key="step.title"
            class="relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-50/80 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-lg"
          >
            <div class="absolute -left-12 top-4 flex h-24 w-24 items-center justify-center rounded-full bg-primary-100/60 text-3xl font-bold text-primary-500">
              {{ index + 1 }}
            </div>
            <div class="ml-14">
        ...
        <aside class="flex flex-col justify-between rounded-3xl border border-primary-100 bg-primary-50/60 p-8 text-primary-900 shadow-glow">
          ...
          <dl class="mt-8 space-y-5">
            <div class="flex items-center justify-between">
              <dt class="text-sm font-medium text-primary-900/70">本周检测文档</dt>
              <dd class="text-xl font-semibold">482 份</dd>
            </div>
            <div class="flex items-center justify-between">
              <dt class="text-sm font-medium text-primary-900/70">高风险段落</dt>
              <dd class="text-xl font-semibold">12 处</dd>
            </div>
            <div class="flex items-center justify-between">
              <dt class="text-sm font-medium text-primary-900/70">润色完成率</dt>
