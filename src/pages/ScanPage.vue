            ...
            <transition name="fade">
              ...
              >
                <button type="button" class="more-menu-item" @click="navigateMorePanel('statistics')">
                  <div>
                    <p class="text-sm font-semibold text-slate-700">Statistics</p>
                    <p class="mt-1 text-xs text-slate-500">View your scanning analytics and usage.</p>
                  </div>
                  <ChevronRightIcon class="h-4 w-4 text-slate-400" />
                </button>
                <button type="button" class="more-menu-item" @click="navigateMorePanel('team')">
                  <div>
                    <p class="text-sm font-semibold text-slate-700">Create a Team</p>
                    <p class="mt-1 text-xs text-slate-500">Create a team to share words with others.</p>
                  </div>
                  <ChevronRightIcon class="h-4 w-4 text-slate-400" />
                </button>
                <button type="button" class="more-menu-item" @click="navigateMorePanel('api')">
                  <div>
                    <p class="text-sm font-semibold text-slate-700">API</p>
                    <p class="mt-1 text-xs text-slate-500">API that detects whether text was generated by AI.</p>
                  </div>
                  <ChevronRightIcon class="h-4 w-4 text-slate-400" />
                </button>
              </div>
...
const onGlobalClick = (event) => {
  ...
  if (
    newMenuOpen.value &&
    !newMenuRef.value?.contains(target) &&
    !newMenuButtonRef.value?.contains(target)
  ) {
    newMenuOpen.value = false;
  }
  if (
    ...
    !moreMenuButtonRef.value?.contains(target)
  ) {
    moreMenuOpen.value = false;
  }
};
...
.more-menu-item {
  @apply flex w-full items-center justify-between gap-3 rounded-xl px-3 py-3 text-left transition hover:bg-slate-100;
}
